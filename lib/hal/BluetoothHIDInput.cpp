...
void BluetoothHIDManager::updateActivity() {
  ...
  // Fire pending GameBrick synthetic releases first — they are due ~80ms
  // after the press, so they must not wait for the 10s maintenance window.
  for (auto& device : _connectedDevices) {
    if (device.pendingGameBrickRelease && now >= device.pendingGameBrickReleaseMs) {
...
void BluetoothHIDManager::checkAutoReconnect(bool userInputDetected) {
  ...
  // Latch user input between rate-limited checks so a press in any frame
  // still counts when the 5s window next opens.
  static bool recentUserInput = false;
  ...
  static bool attemptedReconnect = false;
  unsigned long now = millis();
  ...
  // The settings UI owns the radio during a scan — don't steal it
  if (_scanning) {
  ...
  // connectToDevice blocks the UI for up to 10s, so only attempt while the
  // user is actively pressing buttons (i.e. expects the remote to work now).
  if (!recentUserInput) {
    return;
  }

  // Avoid reconnect storms — wait 30s between attempts (first one is free)
  if (attemptedReconnect && now - lastReconnectAttempt < 30000) {
    return;
  }

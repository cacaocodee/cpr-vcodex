...
bool BluetoothHIDManager::connectToDevice(std::string address) {
  ...
  // Look up address type + name from our own scan results. NimBLE's internal
  // cache is always empty here — startScan uses setMaxResults(0) — so the
  // type captured in onScanResult is the only record of it.
  ...
  for (const auto& dev : _discoveredDevices) {
    if (dev.address == address) {
      ...
      LOG_INF("BT", "Found in discovered list: type=%d", addrType);
      break;
    }
  }
  // Fallback: use saved address type from bonded device
  if (addrType == 0xFF && address == _bondedDeviceAddress && _bondedDeviceAddrType != 0xFF) {
  ...
  // Delete stale bond before connecting — prevents NimBLE security-state crashes
  // when reconnecting to a previously bonded device whose keys have changed.
  NimBLEAddress bleAddress(address, addrType);
  if (NimBLEDevice::isBonded(bleAddress)) {
    LOG_INF("BT", "Deleting stale bond for %s before reconnect", address.c_str());
  ...
  // Some keyboards don't expose Report characteristics but support Boot Protocol
  if (reportChars.empty()) {
    LOG_INF("BT", "No Report chars found, trying Boot Keyboard Input (0x2A22)...");
    auto* pBootChar = pService->getCharacteristic("2A22");
    if (pBootChar && (pBootChar->canNotify() || pBootChar->canIndicate())) {
      reportChars.push_back(pBootChar);
      LOG_INF("BT", "Found Boot Keyboard Input characteristic as fallback");
    }
  }

  if (reportChars.empty()) {
    lastError = "No input report characteristic found";
    LOG_ERR("BT", "No Report or Boot Keyboard characteristic found");
    pClient->disconnect();
    return false;
  }

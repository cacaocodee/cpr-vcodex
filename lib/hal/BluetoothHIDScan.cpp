...
void BluetoothHIDManager::startScan(uint32_t durationMs) {
  ...
  // Active scan: many page-turner remotes (e.g. iiNE Game Brick) only carry
  // their name/HID UUID in the scan response, which passive scanning never
  // requests — they would be filtered out as anonymous non-HID devices.
  // Costs ~4KB heap, but scans only run inside the pairing screen.
  pScan->setActiveScan(true);
  // Callbacks-only (no internal result storage): we deduplicate into our own
  // capped vector, and NimBLE's storage cap could otherwise fill with
  // phones/earbuds before the remote is seen in crowded environments.
...
void BluetoothHIDManager::onScanResult(NimBLEAdvertisedDevice* advertisedDevice) {
  ...
  // Rescue known remotes that advertise anonymously: a MAC-prefix match in
  // the device profile table (e.g. iiNE Game Brick V2 = 60:4d:ec) counts as
  // HID even when the advertisement carries no name or HID service UUID.
  if (!isHID && DeviceProfiles::findDeviceProfile(address.c_str(), hasName ? name.c_str() : nullptr)) {
    isHID = true;
  }

...
void BluetoothSettingsActivity::handleMainMenuInput() {
  ...
  // Fork navigation convention: Up/Left = previous, Down/Right = next
    ...
    case TOGGLE_BT:
      if (btMgr->isEnabled()) {
        ...
        if (btMgr->enable()) {
          ...
          SETTINGS.applyBleMemoryOverrides();
          SETTINGS.saveToFile();
          // Free the stats history for NimBLE; releasing saves it first, and
          // suspending persistence keeps the (now empty) store from ever
          // overwriting the stats file while Bluetooth is on.

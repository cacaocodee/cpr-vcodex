...
void BluetoothSettingsActivity::handleDeviceListInput() {
  ...
  // (Back returns to the main menu via the shared loop() handler, and the
  // "< Rescan >" list row triggers rescans.)
  ...
  if (selectedIndex >= 0 && selectedIndex < static_cast<int>(devices.size())) {
    ...
    if (btMgr->connectToDevice(devAddr)) {
      ...
      SETTINGS.saveToFile();
      // Idempotent when the enable toggle already released the stats store
